...
<body>
	...
	<div id="index_layout">
		...
		<div data-options="iconCls:'icon-logo',region:'center'"
			...
			<c:forEach var="device" items="${devices}" varStatus="index">
				<c:if test="${index.count==1 }">
				<div title="${device.deviceName}" data-options="href:'${ctx}/device/devicelist/${device.id}',iconCls:'icon-webcam'"></div>
				</c:if>
			</c:forEach>
			</div>
		</div>
